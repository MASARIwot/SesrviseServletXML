...
public class URLFileWriter {
	...
	public URLFileWriter(String path, ConcurrentLinkedQueue<String> urlShema,
			int number) {
		this.path = path;
		this.number = number;
	...
	private void writeShema() throws IOException {
		try {
			String charCounter;
			bufWriter
					.write(charCounter = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
			bufString.append(charCounter);
			bufWriter.newLine();
			bufWriter
					.write(charCounter = "<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">");
			bufString.append(charCounter);
			bufString.append(charCounter);
			bufWriter.newLine();
			bufWriter.flush();
			...
			while (!urlShema.isEmpty()) {
				...
				if (beanUrl.getLastmod() != null) {
					bufWriter.write(charCounter = "	<lastmod>"
							+ beanUrl.getLastmod() + "</lastmod>");
					bufString.append(charCounter);
					bufWriter.newLine();
					bufWriter.newLine();
				}
				if (beanUrl.getChangefreq() != null) {
					bufWriter.write(charCounter = "	<changefreq>"
							+ beanUrl.getChangefreq() + "</changefreq>");
					bufString.append(charCounter);
					bufWriter.newLine();
					bufWriter.newLine();
				}
				if (beanUrl.getPriority() != null) {
					bufWriter.write(charCounter = "	<priority>"
							+ beanUrl.getPriority() + "</priority>");
					bufString.append(charCounter);
					bufWriter.newLine();
					bufWriter.newLine();
				}
		...
		if (iterator == 5242871) {
			FileSession.isDone = false;
			Thread myThready = new Thread(new FileSplitWriter(this.path,
					this.urlShema, this.number));
			myThready.isDaemon();

...
public class FileShemaWriter {
	...
	public static String write(String path, String ServerLocation) throws UnsupportedEncodingException {
		...
		String fileName = "FileShema"+ (Calendar.getInstance().getTime().toString().replace(':', '_').replace(' ', '_').trim())+".xsd";
		File file = null;
		BufferedWriter bufWriter = null;
		try {
			file = new File((path + fileName).trim());//File.createTempFile(fileName, ".xsd");
			bufWriter = new BufferedWriter(new FileWriter(file) );
		} catch (IOException e) {
			e.printStackTrace();
		}
		//cp1251 - windows
		bufString.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"+
		...
		for (String s : fileSession.getFileList()) {
			bufString.append("\n<sitemap>\n");
		//	String ss =  new String(s.getBytes(),"cp1251");
			bufString.append("\n\t\t<loc>" + (ServerLocation+s.replace(path, "")).trim() + "</loc>\n");
			bufString.append("\n\t\t<lastmod>" + new Date((new File(s).lastModified()))
					+ "</lastmod>\n");
			bufString.append("\n </sitemap>\n");
